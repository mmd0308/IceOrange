<template>
  <el-drawer :title="drawerTitle" :before-close="handleCancel" :visible.sync="drawerVisible" direction="rtl" ref="drawer">
    <div class="ice-drawer__content">
      <el-form :ref="dataFormRef" :rules="rules" :model="dataForm" label-position="top">
        ...
        <el-form-item label="备注:" prop="remark">
          <el-input type="textarea" :rows="2" v-model="dataForm.remark">
          </el-input>
        </el-form-item>
      </el-form>
      <div slot="footer" class="ice-drawer__footer">
        <el-button type="primary" size="small" icon="el-icon-check" @click="formStatus==='create'?createData():updateData()">确定
        </el-button>
...
export default {
  data() {
    return {
      ...
      rules: {
        ...
        showName: [
          { required: true, trigger: 'blur', message: '显示名称不能唯空!' }
        ]
      }
    }
  },
  methods: {
    ...
    handleToCreate() {
      ...
      this.drawerTitle = '注册服务'
      this.resetForm()
    },
    handleToUpdate(row) {
      this.drawerVisible = true
      this.drawerTitle = '修改服务'
